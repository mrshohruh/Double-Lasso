...
def _select_shortlist(scenarios: list[str], max_items: int = 5) -> list[str]:
    ...
    explicit = {"baseline", "rho_05", "near_p_equals_n", "heavy_tail_baseline"}
    for sc in present:
        if sc in explicit:
            shortlist.append(sc)
    if shortlist:
    ...
    near = next((sc for sc in present if "near" in lower_map[sc] or "p_equals_n" in lower_map[sc]), None)
    if near:
        shortlist.append(near)
...
def plot_coverage_by_scenario(df: pd.DataFrame, outfile: Path) -> None:
    ...
    ax.legend(title="Configuration", frameon=False, loc="upper center",
              bbox_to_anchor=(0.5, -0.22), ncol=2)
    _style_ax(ax)
    ax.get_figure().tight_layout(rect=[0, 0.1, 1, 1])
